...
public class MatrixChainMultiplication {
    ...
    public static int mcm(int[] arr,int i,int j) {
        ...
        for(int k=i;k<=j-1;k++){
            ...
            int cost1=mcm(arr, i, k);   // Ai..Ak     ->  arr[i-1] X arr[i](A), arr[k-1] X arr[k] == arr[i-1] X arr[k]
            int cost2=mcm(arr, k+1, j); // Ak+1....Kj ->  arr[k]   X arr[k+1] , arr[j-1] X arr[j] == arr[k] X arr[j]
            
            int cost3=arr[i-1]*arr[k]*arr[j];    // cost= a*b*d         
            
            int finalresult=cost1+cost2+cost3;
